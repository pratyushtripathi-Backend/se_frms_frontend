...
export default function DecisionTablePage() {
  ...
  const loadDecisions = useCallback(async () => {
    ...
    try {
      ...
      setDecisionRows(normalizedResponse.rows);
      ...
      setTotalPages(1);
      setErrorMessage(
        getAuthErrorMessage(error, "Unable to load decision table. Please try again."),
      );
    } finally {
      setIsLoading(false);
    }
  ...
  const exportCSV = () => {
    ...
    link.href = url;
    link.download = "decision-table.csv";
    link.click();
  ...
  const styles = {
    ...
    decisionTd: (decision) => ({
      ...
      fontSize: "13px",
      fontWeight: 600,
      color: decisionColors[decision] || "#555555",
      whiteSpace: "nowrap",
    }),
    ...
    <div style={styles.page}>
      <div style={styles.card}>
        <div style={styles.headerRow}>
          <div style={styles.title}>Decision Table Overview</div>
            ...
            <tbody>
              ...
              {!isLoading && !errorMessage && currentRows.map((row) => (
                <tr key={row.srNo} style={styles.tr}>
                  ...
                  <td style={styles.decisionTd(row.finalDecision)}>
                    {row.finalDecision}
                  </td>
                  <td style={styles.reasonTd}>{row.decisionReason}</td>
                  <td style={styles.td}>
...
function normalizeDecisionResponse(responseData, currentPage, pageSize) {
  const payload = responseData?.responseData ?? responseData?.data ?? responseData;
  const rows = findFirstArray(payload).map((row, index) =>
    normalizeDecisionRow(row, (currentPage - 1) * pageSize + index + 1),
  );
  const totalRecords =
...
function normalizeDecisionRow(row, srNo) {
...
function findFirstArray(value) {
  ...
  const directKeys = ["content", "records", "items", "data", "list", "decisions"];
